...
async function checkPendingTransactions() {
  try {
    const latestBlock = await web3.bnb.getBlock('latest');
    console.log(latestBlock)
    const pendingBlockNumber = latestBlock.number;
    // console.log(pendingBlockNumber)
    // console.log(await web3.eth.getBlock('pending', true).transactions);
    console.log(web3.eth.getBlock('pending', true))
    const pendingTransactions = await web3.bnb.getBlock('pending', true).transactions;

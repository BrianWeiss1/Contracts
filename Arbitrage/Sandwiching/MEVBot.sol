...
contract MEVBot {
    ...
    function monitorMempool(address targetAddress) internal {
        require(msg.sender == owner, "Only owner can call this function");
        require(isRunning == true, "The bot is not running");

        bytes32 targetHash = keccak256(abi.encodePacked(targetAddress));
    ...
    function executeOperation(address[] calldata _assets, uint[] calldata _amounts, uint[] calldata premiums, address initiator, bytes calldata params) external returns (bool) {
        ...
        (bytes32 targetHash, uint targetBlock, uint[] memory amounts, address[] memory targetAssets, uint[] memory modes) = abi.decode(params, (bytes32, uint, uint[], address[], uint[]));
        ...
        for (uint i = 0; i < count; i++) {
            ...
            while (true) {
                ...
                if (txHash == targetHash) {
                    ...
                    uint amountOut = amountIn * IERC20(path[1]).balanceOf(address(this)) / IERC20(token).balanceOf(address(this));

                    if (amountOut > amountOutMin) {
                        uint[] memory flashAmounts = new uint[](3); // Fix memory array initialization
                        flashAmounts[0] = amountIn;
                        flashAmounts[1] = amountOut;
                        flashAmounts[2] = 0;
                        ...
                        bytes memory flashParams = abi.encode(amountIn, amountOut, path, flashAssets, flashModes);
